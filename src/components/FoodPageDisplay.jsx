    ...
    <div className='max-w-6xl mx-auto  h-screen'>
        ...
        <div className='flex justify-between items-center p-3 pt-6'>
            <div className='flex space-x-2 mt-3'>
                <button disabled={count<1} onClick={()=> {dispatch(decrement({id}))}} className={darkValue ? 'bg-white text-black rounded-md px-2 font-bold  text-3xl ' : 'bg-slate-300 rounded-md px-2 font-bold text-3xl '}>-</button>
                <button onClick={()=> dispatch(increment({id}))} className={darkValue ? 'bg-white text-black rounded-md px-2 font-bold  text-3xl ' : 'bg-slate-300 rounded-md px-2 font-bold text-3xl '}>+</button>
            </div>
